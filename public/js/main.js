...
(function ($) {
    ...
    $("#transaksi_product_id").on("change", function (e) {
      ...
      $.get(url, function (data, status) {
        if (status == "success" && data != "false") {
          ...
          var val = value[0];
          var sale_value = '<option value="' + val.sale_price + '">' + price(parseInt(val.sale_price)) + " Per Buah</option>"; //Diganti untuk harga
          if (val.sale_price_type1 != "0") {
            var type1 = '<option value="' + val.sale_price_type1 + '">' + price(parseInt(val.sale_price_type1)) + " Per Lusin </option>";
          }
          if (val.sale_price_type2 != "0") {
            var type2 = '<option value="' + val.sale_price_type2 + '">' + price(parseInt(val.sale_price_type2)) + " Per Kilo </option>";
          }
          if (val.sale_price_type3 != "0") {
            var type3 = '<option value="' + val.sale_price_type3 + '">' + price(parseInt(val.sale_price_type3)) + " Per Kodi</option>";
          }
          // $("#sale_price").append(sale_value + type1 + type2 + type3);
          $("#sale_price").append(sale_value);
          $("#sale_price").append(type1);
          $("#sale_price").append(type2);
          $("#sale_price").append(type3);
        }
      });
    });
  });
  $("#tambah-barang").on("click", function (e) {
    ...
    if (product_id !== null && sale_price !== null) {
        ...
        success: function (data) {
          ...
          $.each(res.data, function (key, value) {
            ...
            var display =
              ...
              "</th>" +
              '<td><span class="btn btn-danger btn-sm transaksi-delete-item" data-cart="' +
              key +
              '">X</span></td>' +
              "</tr>";
      ...
      swal("Cancel", "Silahkan isi semua box", "error");
    }
  });
  $(document).on("click", ".transaksi-delete-item", function (e) {
    var rowid = $(this).attr("data-cart");
    $el.faLoading();
    $.get($("base").attr("url") + "transaksi/delete_item/" + rowid, function (data, status) {
      if (status == "success" && data != "false") {
        $("#" + rowid).remove();
        console.log(data);
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (response.status == "ok") {
            ...
            // alert("<div class='alert alert-success' role='alert'> A simple success alert—check it out! </div>");
            // swal("Sukses Input", "You clicked the button!", "success");
            swal("Sukses Input", "You clicked the button!", "success").then((value) => {
              //vale boleh dikosongkan
            ...
            // swal({
            //   text: "Sukses Input",
            //   buttons: ["Stop", "Do it!"], //sudah berhasil namun langsung ganti halaman
            // });
            // window.location.href = $("base").attr("url") + method;
            ...
            swal("Cancel", "Stok jumlah produk yang anda pilih sudah habis", "error");

            // alert("Stok jumlah produk yang anda pilih sudah habis");
          } else {
            swal("Cancel", "Terjadi error di server, silahkan coba lagi", "error");
